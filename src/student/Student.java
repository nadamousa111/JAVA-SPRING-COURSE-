...
public class Student {
    ...
    public void displayHighestGrade(Student[] grades) {
        double max = grades[0].getStudentGrade();
        String studentName = grades[0].getStudentName();
        for (int i = 1; i < grades.length; i++) {
            if (grades[i].getStudentGrade() > max) {
                max = grades[i].getStudentGrade();
                studentName = grades[i].getStudentName();
            }

        }
        System.out.println("The highest grade is " + max + " and the student with the highest grade name is "+ studentName);
    }
